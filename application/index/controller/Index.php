...
class Index extends Base{
    ...
    public function index(){
		...
		$title = "mark";
      	$c = file_get_contents('./data/'.$title.'.md');
		$style = <<<EOF
		<!DOCTYPE html>
<html lang='en'>
<head>
...
EOF;
		...
		$all = face_detect($image,'./data/haarcascades/haarcascade_frontalface_alt.xml');
		$imagick = new \Imagick($image);  
	    if (is_array($all)) {  
	        foreach ($all as $v) {  
	            $imagick->cropImage($v['w'], $v['h'], $v['x'], $v['y']);  
...
//				$im->compositeImage( $im_cl, Imagick::COMPOSITE_OVER , $v['x'], $v['y'] ); 
	    ...
	    ob_clean();  
		//p('Content-type:'.strtolower($imagick->getImageFormat()));
	    echo $imagick;  
	    $imagick->destroy(); */ 
	}
	/**
